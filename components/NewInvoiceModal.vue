...
const { isLoading, invoiceDialog, invoicesLoaded, editInvoice, filterResults } = storeToRefs(store);
...
const currentInvoice = ref(editInvoice.value ? await useInvoice(id) : null);
...
const invoiceObject = reactive({
  // invId: 1,
  invId: invoices.value ? invoices.value.length + 1 : 1,
  clientCompany: '',
  clientName: null,
  clientName2: null,
  clientEmail: null,
...
// const { uploadToSupabase, updateOnDatabase } = useInvoice();
...
<template>
  ...
  <div class="modal">
    <div
      ref="modal"
      class="modal-box relative h-full dark:border dark:border-dark-medium dark:bg-dark-strong lg:min-w-min lg:max-w-3xl"
    >
      <label ref="closeBtn" for="my-modal-3" class="hidden">✕</label>
      <button
        type="button"
        @click="toggleModal"
        class="btn-primary btn-sm btn-circle btn absolute right-2 top-2 z-50 text-light-medium hover:bg-primary/50 dark:border-primary/50 dark:bg-primary/50 dark:hover:bg-primary"
      >
        X
      </button>
      <form @submit.prevent="submitForm" class="scrollbar-form relative w-full">
        ...
        >
          {{ !editInvoice ? 'Nueva Cotización' : 'Editar Cotización' }}
        </h1>
            ...
            <div class="dropdown-bottom dropdown form-control relative mb-4 h-full w-1/2 items-end">
              ...
              <div class="form-control relative flex w-full flex-row">
                <button
                  ...
                  :class="{
                    pending: invoiceObject.status === 'Pendiente',
                    draft: invoiceObject.status === 'Borrador',
                  }"
                >
                  <Icon
                    :name="
              ...
              <!-- <div v-if="filteredContacts && contactsModal" @click="contactsModal = false"></div> -->
              <div class="w-full">
                <ul
                  v-if="statusModal"
              ...
              <table class="w-full">
                ...
                >
                  ...
                  >
                    ...
                    <td class="absolute -top-5 right-0" v-if="invoiceObject.invoiceItem.length > 1">
                      <Icon
